...
type DownloadEvent = { kind: "started" | "log" | "error" | "completed" | "failed" | "cancelled"; message: string };
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = listen<DownloadEvent>("download-event", ({ payload }) => {
      ...
      if (["started", "completed", "failed", "cancelled"].includes(payload.kind)) void refreshStatus();
    });
  ...
  const start = async () => { if (!settings) return; setBusy(true); setError(""); setNotice(""); setLogs([]); try { await invoke("start_download", { settings }); setRuntime((current) => ({ ...current, running: true })); } catch (reason) { setError(String(reason)); } finally { setBusy(false); } };
  ...
  return <main className="app-shell">
    ...
    </section><aside className="side-column"><div className={`action-card ${runtime.running ? "is-running" : ""}`}><div className="action-icon"><Icon name={runtime.running ? "terminal" : "download"} /></div><h2>{runtime.running ? "Идёт загрузка" : "Всё готово?"}</h2><p>{runtime.running ? "Окно можно оставить открытым — прогресс появится ниже." : "Проверьте автора, доступ и выбранные материалы."}</p>{runtime.running ? <button className="danger primary-action" onClick={stop}><Icon name="stop" />Остановить</button> : <button className="primary primary-action" onClick={start} disabled={!canStart || busy}><Icon name="download" />Начать загрузку</button>}<button className="save-button" onClick={save} disabled={busy}>Сохранить настройки</button><div className="action-links"><button onClick={() => openPath(settings.destinationDirectory)}><Icon name="folder" />Открыть папку</button><button onClick={() => openUrl("https://github.com/Glitchy-Sheep/boosty-downloader")}><span>↗</span>Документация</button></div></div><div className="log-card"><div className="log-header"><div><Icon name="terminal" /><b>Журнал</b></div>{logs.length > 0 && <button onClick={() => setLogs([])}>Очистить</button>}</div><div className="log-body">{logs.length === 0 ? <div className="log-empty"><span>›_</span><p>Здесь появится ход загрузки</p></div> : logs.map((line) => <div key={line.id} className={`log-line ${line.kind}`}><span>{line.kind === "error" || line.kind === "failed" ? "!" : "›"}</span><p>{line.message}</p></div>)}<div ref={logEnd} /></div></div></aside></div>
